...
def _get_ec2_hostinfo(path="", data={}):
    ...
    """
    try:
        minions = _call_aws("/latest/meta-data/%s" % path).split("\n")
...
def ec2_info():
    ...
    try:
        ...
        try:
            iam_role = json.loads(grains['ec2_iam_info'])['InstanceProfileArn']
        except (KeyError, TypeError, ValueError):
            iam_role = None
        if iam_role:
            # arn:aws:iam::12345:instance-profile/example-production-role-name
            iam_role_name = iam_role.split('/')[-1]
            grains['iam_role_name'] = iam_role_name
        return grains
